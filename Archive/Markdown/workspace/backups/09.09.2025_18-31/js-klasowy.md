...
class ChatUI {
   ...
   * @param {'user'|'ai'} type - Typ wiadomości (user lub ai).
  ...
  /**
   * Przewija widok do ostatniej wiadomości.
   */
  scrollToBottom() {
    if (this.dom.chatContainer) {
  ...
  /**
   * Dodaje wiadomość z HTML (np. z backendu z formatowaniem).
   * @param {'user'|'ai'} type - Typ wiadomości.
   * @param {string} html - Treść wiadomości jako HTML.
   */
  addMessageHTML(type, html) {
    if (!this.dom.chatContainer) return;
...
class TagsPanel {
   ...
   */
    ...
    if (!(container instanceof HTMLElement)) {
      const actualType = container === null
        ? 'null'
...
class App {
  constructor() {
    ...
    // 3. Moduły zależne od tag panelu
    this.tagsPanel = null;
...
renderTagPanel(targetSelector = '#side-panel') {
  ...
  init() {
    ...
    // Inicjalizacja globalnych modułów
...
class ChatManager {
  ...
  async sendPrompt() {
    ...
    // Blokujemy input i czyścimy pole
    ...
    // Dodajemy wiadomość AI z animacją czasu
    ...
    try {
      ...
      // Dodajemy formularz oceny
      this.chatUI.addRatingForm();
    } catch (err) {
      clearInterval(timer);
